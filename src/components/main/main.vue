<template>
  <Layout style="height: 100%" class="main">
    ...
    <Layout>
      <Header class="header-con">
        <header-bar :collapsed="collapsed" @on-coll-change="handleCollapsedChange">
          ...
          <fullscreen v-model="isFullscreen" style="margin-right: 10px; position: absolute; left:50px; top: -2px;"/>
        </header-bar>
      </Header>
    ...
    :title="getModalTitle"
    footer-hide
    :width="$store.state.modal.width || 1000"
...
import TravelSet from '@/view/company/travel_set'
export default {
  name: 'Main',
  components: {
    SideMenu,
  ...
  methods: {
    ...
    handleCloseTag (res, type, route) {
      if (type === 'all') {
        ...
        if (type !== 'others') {
          let nextRoute = getNextRoute(this.tagNavList, route)
          if(!nextRoute.hasOwnProperty('name')){
            nextRoute = {
                path: '/memberList',
                name: 'memberList',
                meta: {
