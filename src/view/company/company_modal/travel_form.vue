...
<template>
  <Form ref='formValidate' :model="formValidate" :rules="ruleValidate" label-position="left" :label-width="70">
    <Row :gutter="15">
        <Col span="6">
      <FormItem label="差旅等级" prop="level">
        <Input  v-model="formValidate.level"  placeholder="请不要输入过长"></Input>
      </FormItem>
      </Col>
        <Col span="6">
      ...
      <!-- <Col span="6">
      <FormItem label="等级类型" prop="levelType">
        <Select filterable v-model="formValidate.levelType" placeholder="请选择审批类型">
            <Option  v-for="item in travelList"   :value="item.id" :key="item.id">{{ item.name }}</Option>
        </Select>
      </FormItem>
      </Col> -->
...
export default {
  ...
  methods: {
    ...
    submiteData (formValidate) {
      ...
      //  }
      this.$store.commit('setCurUrl', `/api/companyTravelLevel/list?companyId=${this.$route.query.id}`)
      ...
      Axios({
        ...
        if (res.data.code === 200) {
          ...
          this.$Message.error('差旅等级已存在 请检查')
        }
      })
    }
  },
